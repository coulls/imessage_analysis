...
for i in range(250):
    ...
    start_capture_string = "%s/other/data_start/%d.pcap" % (OSX_RECV_DATA,i)
    try:
        os.remove(start_capture_string)
    except:
        pass

    stop_capture_string = "%s/other/data_stop/%d.pcap" % (OSX_RECV_DATA,i)
    try:
    ...
    #Run applescript and finish
    applescript_cmd = "ssh -i /Users/coulls/.ssh/id_rsa coulls@scoull.rj \"osascript %s/del_letter.scpt 100\"" % (OSX_RECV_CODE)
    subprocess.call(shlex.split(applescript_cmd))
